<?php
	...
	$compile .= '<a class="button_size_'. esc_attr($button_size) .' ' . esc_attr($btn_icon_position_class) . ' ' . esc_attr($css_class) . ($is_lwa ? ' lwa-links-modal lwa-login-link' : '') . '" href="'.esc_attr($url).'" '.$title_for_button.' '.$button_target.' ' . (strlen($btn_style) ? 'style="' . esc_attr($btn_style) . '"' : '') . ' ' . $btn_attr . '>' . $btn_value . '</a>';
