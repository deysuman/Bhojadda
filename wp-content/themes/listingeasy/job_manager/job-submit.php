...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	<?php if ( job_manager_user_can_post_job() ) : ?>
		...
		uasort( $all_fields, 'gt3_sort_array_by_priority' ); ?>
		...
		<?php foreach ( $all_fields as $key => $field ) : ?>
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<label for="<?php echo esc_attr( $key ); ?>">
					...
					echo apply_filters( 'submit_job_form_required_label', (isset($field['required'])&&$field['required']) ? '' : ' <small>' . esc_html__( '(optional)', 'listingeasy' ) . '</small>', $field );
					?>
				</label>
