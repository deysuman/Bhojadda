...
{
    load_theme_textdomain('listingeasy', get_template_directory() . '/core/languages/');
}
...
class GT3_Walker_Dashboard_Menu extends Walker_Nav_Menu {
    ...
    public function end_el( &$output, $item, $depth = 0, $args = array() ) {
        ...
        if ( $item->menu_item_parent == '0' && $this->elements_counter == $this->elements ) {
            $logout_link = get_permalink();
            $logout_link = $logout_link == 0 ? home_url() : $logout_link;
            $output .= "<li class=menu-item><a href='".esc_url(wp_logout_url( $logout_link ))."'>".esc_html__( 'Log Out', 'listingeasy' )."</a></li>{$n}";
        }
    }
...
// Max Width of 450px
add_image_size( 'gt3-card-image', 450, 9999, false );
add_image_size( 'gt3-carousel-image', 9999, 800, false );
add_image_size( 'gt3-featured-image', 2700, 9999, false );